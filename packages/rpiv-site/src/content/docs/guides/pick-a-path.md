...
- **`/wf ship`**: `goal → research → plan → plan-cite-check → grade → implement → implement-scope-check → reconcile → validate → (validate-fix, once) | commit` (11 stages). The lightweight preset: a small, well-understood task in one forward pass — trimmed research, a single unsliced plan from `quick-plan`, one three-dimension grade, stop-on-fail at every gate (hand-repair and resume with `/wf @<run-id>` to re-run the halted gate) except validate's single bounded remediation hop. The opposite end of the spectrum from `build`: no slicing, no parallel design, no confirm panels. → [Run a workflow](/docs/guides/run-a-workflow).
