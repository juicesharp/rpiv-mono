...
key_steps:
  ...
  - title: Cut vertical, independent, right-sized slices
    rationale: Horizontal layers ("all the types") are valuable only once combined, so each slice must be a user- or system-meaningful capability resolving to one coherent architecture decision — the exact bar the design-readiness gate later judges.
