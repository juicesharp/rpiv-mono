...
key_steps:
  - title: Read the whole plan, own one phase
    rationale: The `## Synthesis Notes` carry the reconciled cross-phase seams. Sibling phases' interfaces must be referenced by the shape `synthesize` already fixed. Skimming them is required precisely so their code is never rewritten here.
