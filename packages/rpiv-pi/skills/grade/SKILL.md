...
- `--dimension <name>` **(required)** — one of:
  ...
  - **slice-breakdown dimensions** (a slice map — the sizing gate): `right-sizing`, `independence`, `verifiability`, `vertical-shape`.
...
| `right-sizing` | Each slice is independently implementable in roughly **4–8 hours** (≤ ~1 day) — no epic (spans multiple subsystems, bundles capabilities via "and"/"or"/"manage", or oversized scope) and no noise fragment (<~1h, should merge); the brief is actually decomposed (more than one slice, not over-atomized). | The slice map's `slices:` array and each `## Slice N:` Scope; **spot-check** the cited `file:line`s to gauge real effort. | Every slice sits in the ~4–8h band (or names a justified exception), and `slice_count` > 1 without being over-atomized. |
| `independence` | Slices can be implemented and merged in **any order** — no two slices edit the same file/module (overlap), no dependency cycle (A→B→A), and any shared interface/schema is defined by exactly one slice (contract-first). | Each slice's File map / Draws on / Depends on, **cross-referenced across all slices**. | No cross-slice file overlap and no `depends_on` cycle; every shared contract has a single owning slice. |
| `verifiability` | Each slice carries a concrete, checkable "done" — at least one success/acceptance criterion stated as observable behavior (precondition→action→result), with no unmeasurable adjective ("robust", "clean", "fast") left without a metric. | Each slice's Success criteria. | Every slice has ≥1 concrete, checkable criterion. |
| `vertical-shape` | Each slice delivers an **observable end-to-end change** (a capability a caller/user can see), not a horizontal layer/tech task ("build the schema", "wire up the UI") valuable only once combined with sibling slices. | Each `## Slice N:` title + Scope. | No slice is a pure single-layer task with no standalone observable behavior, or the divergence is justified (e.g. one foundational slice). |
