...
- `--cite-check <path>` *(optional, `correctness` only)* — the deterministic citation floor's verdict JSON for the same artifact, passed when the floor recorded advisory findings (citation ambiguity or line drift; a fabricated path already stopped the workflow before you were dispatched). Treat its `findings[]` as **leads to fold into your spot-check sample**, not conclusions — the Citation-resolution rule (step 4) alone decides whether any becomes a finding of yours. Every other dimension ignores this flag; it never triggers confirm mode and never produces `finding_rulings`. Missing or unreadable path → grade normally without it.
