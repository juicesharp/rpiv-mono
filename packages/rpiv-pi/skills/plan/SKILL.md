...
**Exit-0 contract (the `reconcile` stage re-runs AV lines verbatim):** after implement, `reconcile` re-executes the FIRST backtick span of every `- [ ]` line in `#### Automated Verification:` via `execFileSync` — arg-array (no shell: no `|`/`&&`/redirection), stdin ignored, exit 0 the ONLY pass signal. Prose around the span never executes, and a non-zero exit halts the run with no fix arm. So every AV line must be ONE self-contained command that exits 0 exactly when the criterion holds: name the target file INSIDE the span (never "over `<file>`" in prose — the command then greps an empty stdin and always fails); never assert absence ("returns nothing" — grep exits 1 on no match, so that line can ONLY fail; restate it to exit 0 on success, e.g. a `node -e` probe that inverts the check, or move it to `#### Manual Verification:`); never rely on prose expected counts ("returns `4`" — fold the assertion into the command itself); never point a formatter at paths outside its configured scope (Biome on a doc-only phase's `.md` exits non-zero with "No files were processed"). The `plan-cite-check`/`code-cite-check` floor lints these shapes and routes violations into a plan-fix round; anything it cannot catch statically still kills the run at `reconcile`.
