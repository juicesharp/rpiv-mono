...
6. **Print the artifact path**, then a one-line summary: `refined for <dimensions>: <k> findings addressed`.
...
- **Fix the artifact, never the code.** Reading code to ground an edit is fine; editing code is out of scope — `implement` owns that.
